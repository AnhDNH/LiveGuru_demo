...
import Pages.UserCartPO;
import Pages.UserCheckoutPO;
...
public class User_04_Purchase extends WebKeywords {
	WebDriver driver;
...
//	@Test
	public void TC_04_Purchase() {
		...
		log.info("Purchase - Step 10: Verify User firstname & last name");
		verifyEquals(checkoutPage.getUserFirstName("value"), "Ha Anh");
		verifyEquals(checkoutPage.getUserLastName("value"), "Do");

		log.info("Purchase - Step 11: Enter user address with value '" + PurchaseData.BillingAddress.ADDRESS + "'");
		checkoutPage.inputUserAddress(PurchaseData.BillingAddress.ADDRESS);

		log.info("Purchase - Step 12: Enter City with value '" + PurchaseData.BillingAddress.CITY + "'");
		checkoutPage.inputCity(PurchaseData.BillingAddress.CITY);

		log.info("Purchase - Step 13: Select State dropdown with value '" + PurchaseData.BillingAddress.STATE + "'");
		checkoutPage.selectStateDropdown(PurchaseData.BillingAddress.STATE);

		log.info("Purchase - Step 14: Enter ZIP textbox with value '" + PurchaseData.BillingAddress.ZIP + "'");
		checkoutPage.inputZIP(PurchaseData.BillingAddress.ZIP);

		log.info("Purchase - Step 15: Select Country dropdown with value '" + PurchaseData.BillingAddress.COUNTRY + "'");
		checkoutPage.selectCountryDropdown(PurchaseData.BillingAddress.COUNTRY);

		log.info("Purchase - Step 16: Enter Telephone textbox with value '" + PurchaseData.BillingAddress.TELEPHONE + "'");
		checkoutPage.inputTelephone(PurchaseData.BillingAddress.TELEPHONE);

		log.info("Purchase - Step 17: Click on 'Ship to this address' radio button");
		checkoutPage.clickShipToThisAddressRadio();

		log.info("Purchase - Step 18: Click 'Continue' button");
		checkoutPage.clickBillingContinueButton();
		

		log.info("Purchase - Step 19: Verify Shipping method & Flat rate equals '" + PurchaseData.Order.FLAT_RATE + "'");
		verifyEquals(checkoutPage.getShippingMethod(), "Flat Rate");
		verifyEquals(checkoutPage.getFlatRate(), PurchaseData.Order.FLAT_RATE);

		log.info("Purchase - Step 20: Click 'Continue' button");
		checkoutPage.clickShippingContinueButton();

		log.info("Purchase - Step 21: Select 'Check/Money order' radio button");
		checkoutPage.selectCheckMoneyOrderRadio();

		log.info("Purchase - Step 22: Click 'Continue' button");
		checkoutPage.clickPaymentContinueButton();

		log.info("Purchase - Step 23: Verify product details under Order review");
		...
		log.info("Purchase - Step 24: Verify Subtotal, Flat rate, Grand total under Order review");
		...
		log.info("Purchase - Step 25: Click 'Place Order' button");
		checkoutPage.clickPlaceOrderButton();

		log.info("Purchase - Step 26: Verify order received message displayed on page tittle");
		verifyEquals(checkoutPage.getOrderSuccessPageTitleMessage(), "YOUR ORDER HAS BEEN RECEIVED.");

		log.info("Purchase - Step 27: Verify order ID was generated");
